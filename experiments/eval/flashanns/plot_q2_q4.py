...
def _line(ax: Any, rows: list[dict[str, Any]], metric: str, title: str) -> None:
    for system in sorted({r["system"] for r in rows}):
        group = sorted((r for r in rows if r["system"] == system and r.get(metric, "") != ""), key=lambda r: float(r["L"]))
        if not group:
            continue
        x = [float(r["L"]) for r in group]
...
def render_figures(csv_path: Path, out_dir: Path) -> None:
    ...
    for ax, metric, title in zip(axes.flat, ("qps_median", "latency_p99_ms_median", "critical_wait_ms_median", "score_window_pct_median"), ("Throughput", "P99 latency", "Critical wait", "Window score source")):
    ...
    _line(axes[0], [r for r in rows if r["phase"] == "q3_t1"], "qps_median", "T=1 transfer ablation")
    _line(axes[1], [r for r in rows if r["phase"] == "q3_t8"], "qps_median", "T=8 scheduler ablation")
    for ax in axes:
        ax.legend(fontsize=7)
    fig.savefig(out_dir / "q3-ablation.pdf")
    plt.close(fig)

    q4 = [r for r in rows if r["phase"] == "q4"]
