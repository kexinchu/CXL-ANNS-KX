...
> **2026-09-02 起，执行清单改走**
> [`docs/superpowers/plans/2026-09-02-flashanns-paper-plan.md`](../superpowers/plans/2026-09-02-flashanns-paper-plan.md)。
> 本文件保留历史论述、事实墙和 Related 切割表；§0 旧 one-copy 主线、§4.4 LFU 热集、§6–§10 旧里程碑不再当本周任务。
...
1. **带宽瓶颈下，CXL–SSD 结构里哪些带宽是 shared，哪些是各自独立？**
...
## 0. 论文故事线（新-降级）——2026-09-02 锁定

- **核心重点：** flash-backed CXL memory tier 上的 graph ANNS **如何可用**。
- **Oracle 必须拆两层，禁止写成已经一起达到：**
  - **Score hide（已到）：** 全精度打分 100% 来自一小块窗口（`from_win=100`）。host 打分指令看不见 NAND。
  - **Query hide（oracle，未到）：** query 临界路径不因 miss 读 NAND、也不阻塞等 prefetch。T=1 锁定行是 **50.25 / 19.8 ms vs 窗口 oracle 85.78 / 11.4 ms = 0.59×**。残差是指针追逐：hop *i* 打完分才知道 hop *i+1* 的向量页。
- **对照，不是要替代：** DiskANN/PipeANN = 优秀 **block-SSD ANNS**（PQ 导航、sector、AIO、node cache）。CXL-ANNS/Cosmos = **full-in-memory CXL-DRAM ANNS**。本文不替代前者，也不把全库抬进后者。
- **有范围的 first：** 第一个在 SSD-backed CXL 地址空间上直接服务 graph ANNS、并联合管理 residency / promote / search stall 的 runtime。不要写成「第一个 CXL-SSD 向量库」或「第一个 one-copy 多机 ANNS」。
- **系统贡献收口：** 语义放置、有界升迁、page-bin/expand-bundle、异步 prefetch + 跨 query issue。目标是缩小 **naive CXL-SSD ↔ 窗口 oracle**，不是逼近 DiskANN，也不是评估 Shared FAM。
- **执行：** 章节合同、必出图、必跑 ablation、本周不做的刀 → [`2026-09-02-flashanns-paper-plan.md`](../superpowers/plans/2026-09-02-flashanns-paper-plan.md)。
...
- 当前设计组合（CXL–SSD path）：
  - **ANNS-aware page management / layout packing：** 不把 graph node 随机落到 SSD page，而是按图邻接关系与搜索共访问关系重排 layout，把一个中心点的 high-probability neighbors / next-hop candidates 尽量合并到同一个 SSD page 中。这样一次 4 KB miss 不只取回当前 node，也取回后续 beam search 很可能访问的邻居，减少“每跳一次随机 miss”的放大；同时控制 page 内只放语义相关节点，避免为了合并读而把无关数据带入 CXL–SSD device DRAM cache。
  - **Bounded promotion / hot-page residency：** 将 entry points、上层/高频节点、PQ/codebook、以及反复命中的 packed pages 保留在快层；promotion 以 page 为单位但由 ANNS 语义触发，限制升迁预算，避免 naive page cache 被低复用随机遍历污染。
  - **Continuous batching / stall-aware scheduling：** 不按单 query 串行等待 SSD miss，而是持续维护一批 in-flight queries；某个 query 因 CXL–SSD miss stall 时，调度器切到其他 ready queries，并把同一时间窗口内的 page requests 合并、去重、批量发出。目标是填补 pointer-chasing search 留下的 I/O pipeline bubbles，同时避免 search width 盲目增大导致更多无效 SSD reads。
  - **Prefetch / read coalescing with semantic guard：** 只对 page 内 neighbors 和高置信 next-hop 做有限预取；合并读可以提升带宽利用率，但必须受 cache pollution 和无效 neighbor ratio 约束。
- 初步结果：


| Threads | CXL–SSD design QPS | Oracle QPS | 达到 oracle 比例 |
| ------- | ------------------ | ---------- | ------------ |
| 1       | 50.25              | 85.78      | 58.6%        |
| 4       | 113.71             | 136.23     | 83.5%        |


- 解读：T=1 时仍有明显单 query pointer-chasing / miss-latency stall，说明 page packing 和有限预取还不能完全隐藏 CXL–SSD miss；
- T=4 时 continuous batching 开始发挥作用，通过跨 query 填补 bubble，将 QPS 拉到 oracle 的 83.5%。这支持当前论文主线：CXL–SSD 的关键不是简单 mmap / load-store，而是 **page residency + semantic packing + continuous batching** 共同把 memory-semantic flash 的 stall 控制住。
...
\mathrm{MediaCost}*{\text{CXL-DRAM}} \sim D \cdot c*{\mathrm{DRAM}}
...
\mathrm{MediaCost}*{\text{CXL-SSD}} \sim n*{\mathrm{HA}} D \cdot c_{\mathrm{flash}} + H \cdot c_{\mathrm{cache}} + C_{\mathrm{fabric/controller}}
...
```
机会：CXL–SSD = flash 价容量 + 小 memory-semantic 窗口
      （相对 DiskANN replica 少复制税；相对 full CXL-DRAM 冷尾不付 DRAM 价）
      one-copy / Shared FAM 是部署前提，不是本轮实验结果
        ↓
问题：统一 VA 不自动变快。140× miss cliff、页与图错配、盲预取污染窗口、
      best-first 指针追逐把 QD 打到 ≈1
        ↓
贡献：语义放置 + 有界升迁 + hide engine（异步 fill，打分只从窗口）
      缩小 naive CXL-SSD 与窗口 oracle 的差距；PipeANN 是不同合同的残差
```
...
## 10. 下一步（2026-09-02 起改走新 plan）

旧三条（M-E1 / 抽象 Intro / 开 M2/M3 接口）作废。本周只做：

1. **锁 Intro：** score hide ≠ query hide；0.59× 残差写进前两页。
2. **画 Fig.1 / Fig.9，补 Table 2 的 `\ph` 列；用 hide runtime 重跑 ablation（替换 5.35 / 34%）。**
3. **可选：** `--spec-beam-nbrs`，不改 claim 行除非 nq=20 与 nq=100 同时赢。

详见 [`2026-09-02-flashanns-paper-plan.md`](../superpowers/plans/2026-09-02-flashanns-paper-plan.md) Task 1–6。
...
| 2026-09-02 | **冻结降级故事线：** score hide / query hide 拆开；执行清单迁到 `docs/superpowers/plans/2026-09-02-flashanns-paper-plan.md`；本文件改为材料库 |
