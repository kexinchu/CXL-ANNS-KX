...
机会：为何把 CXL–SSD 引入 ANNS？
      → 解耦「容量成本」与「算力扩展」：one-copy + 闪存价容量 + 可缓存 load/store 热路径
问题：引入之后多出了什么新问题？（本文要解决的）
      → 统一 VA 下的 residency/stall 失控（双台阶、页错配、盲目升迁、beam↔miss）
...
| # | 问题 | 一句话回答 |
|---|------|------------|
| 1 | 为何引入 CXL–SSD？ | DiskANN 扩吞吐要复制/分片整库；CXL-DRAM 让冷数据也付 DRAM 价；CXL–SSD 让多节点共享一份闪存价、可寻址的全精度库（机会）。 |
| 2 | 引入后有何问题？ | 冷读 ~80 µs（~100×）；按页缓存 QPS÷3.6；图乱预取 QPS÷5；加大 beam 命中率↓——**三种「默认用法」都会把 stall 放大**。 |
| 3 | 本文解什么？ | 避免上述自杀式用法：语义缓存单位、有界升迁、共享 cache，并在 iso-recall 下管 beam。 |
...
Billion-scale ANNS 必须同时扩展两个**基本独立**的维度：**语料容量**与**查询吞吐**。现有部署需要：**算力扩展能不能不再绑架整库容量成本**。

#### 为什么 DiskANN 不够

DiskANN 等把「一台查询副本」焊成：

```text
one query replica ≈ CPU + DRAM cache + local SSD index
```

查询量上升、需要 \(R\) 台查询节点时，常见只有两条路：

1. **整库复制：** 每台挂一份 SSD 索引 → 容量成本 \(\approx R \times D\)（\(D\) 为全精度语料体积）；TB 级时复制税不可接受。  
2. **分片：** 每台只持有一部分 → 跨分片 fan-out / 路由 / 负载不均，抬高尾延迟。

即便换 NVMe-oF，也只是把本地盘 I/O 换成网络块 I/O；**每台主机仍各自维护 page/buffer cache**，并没有取消「加算力 ≈ 再铺一份（或再切碎）存储」的税。

#### 为什么 CXL-DRAM 不够

CXL-DRAM（及 CXL-ANNS / Cosmos 一类「全库进共享内存池」）解决了「多计算端点访问同一份数据」的共享问题，但要求**整库**落在 DRAM 类介质上：

- 记 \(D\) = 全精度语料，\(H\) = 有效热点工作集；ANNS 通常 \(H \ll D\)。  
- \(\mathrm{Cost}_{\text{CXL-DRAM}} \approx D \cdot c_{\mathrm{DRAM}}\)：大量几乎不碰的冷向量也占用昂贵 DRAM，造成 **stranded memory** / 功耗与采购浪费。

共享内存池解决了 one-copy，却**没有**把容量定价压到SSD。

#### CXL–SSD 占据的部署点

Fabric-attached CXL–SSD（Shared FAM / GFAM 语义）让多台查询节点共享**一份**持久、SSD作后端的地址空间：

\[
\mathrm{Cost}_{\text{CXL-SSD}} \approx D \cdot c_{\mathrm{flash}} + H \cdot c_{\mathrm{cache}} + \mathrm{Cost}_{\mathrm{fabric}}
\]

对照复制型 DiskANN：

\[
\mathrm{Cost}_{\text{replicated DiskANN}} \approx R \cdot D \cdot c_{\mathrm{flash}} + R \cdot H \cdot c_{\mathrm{host\text{-}DRAM}}
\]

其价值**不是**「NAND miss 变得和 DRAM 一样快」，而是同时具备三点：

| 属性 | 含义 |
|------|------|
| **One-copy** | 多查询节点共享一份 TB 级全精度库；加 worker 不再复制整库容量 |
| **Flash-priced capacity** | 冷数据留在 NAND；不必把 \(D\) 全体升到 CXL-DRAM 价 |
| **Memory-semantic hot path** | 热点在主机/设备 cache 上 load/store；只有 miss 才打 NAND |

（相关硬件叙事见 From Block to Byte、CXL-AnySSD 等：把 DRAM 热窗口与闪存容量解耦到同一内存语义层级。）

**一句话（Why introduce）：**  
DiskANN 扩吞吐要付整库复制/分片税；CXL-DRAM 让冷数据也付 DRAM 价；CXL–SSD 的独特机会是让多查询节点共享**一份**闪存价、持久、可内存寻址的全精度库——**解耦容量成本与算力扩展**（机会，不是本文系统贡献）。

#### 论据成立的前提（写 Intro 时必须交代）

1. **部署模型是 multi-host / fabric-attached CXL–SSD。** 若只是单机 direct-attach，相对「NVMe + record cache」的差异会被削弱。  
2. **评测必须展示 provisioning 随 \(R\) 的增长曲线**，以及不同 \(H/D\) 下相对「复制 DiskANN / 全库 CXL-DRAM」的 Pareto 交叉点；并检查共享 CXL–SSD 带宽是否成为中心化索引的瓶颈。  
3. 本机当前 `/dev/vmem0`（host-NVMe software 路径）用于 **residency/stall 机制研究**；§1.1 的部署叙事以 fabric one-copy 为最终形态，实验平台需在论文中显式对齐或声明 proxy。
