...
\sys{} is a userspace beam search (\texttt{serving/search\_beam}) over a CXAN packed (or page-binned) image \texttt{mmap}'d from \texttt{/dev/vmem0}.
Policy P3 issues hop-ranked miss pages with \texttt{VMEM\_IOC\_PREFETCH\_BATCH}, copies into a host bounce (\texttt{PageCopyPool}), scores there, and installs only \texttt{install\_top} pages into a 1\,GiB DramWindow.
That split is exactly the hide debt~\S\ref{sec:eval}: scoring still waits on the bounce, so NAND is not yet off the critical path.
...
P0 demand promote and P3 bounded install; \texttt{PREFETCH\_BATCH}; page-bin layout pack; iso-recall $L$ picker; optional graph copy into host DRAM (intended default for hide rows).
